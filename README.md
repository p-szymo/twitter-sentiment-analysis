...
- Looking at the sentiment distribution by month tells a similar story and further visualizes a downward trend in negative tweets through April.

![Sentiment Distribution by Month](charts/sentiment_month_barstack.png)
<br/><br/>
- It is important to note though that positive tweets do remain relatively constant, with April showing instead an increase in the number of neutral tweets, which is most likely a result of wider news coverage.

### 2. Language use for each sentiment

![Word Cloud](charts/word_cloud_all.png)

- The results for frequently used words overal were not too suprising. After removing common English stopwords as well as topical stopwords like mask and virus, the top ten most frequently occuring words were: *hand*, *need*, *spread*, *protect*, *make*, *help*, *say*, *glove*, *public*, and *hospital*.
...
<br/><br/>
- **Negative tweets**
    - A surprising number of medical terms, but also words like *death*, *Trump*, and *China*.
- **Positive tweets**
...
<br/><br/>
- Positive tweets tended to receive more likes, whereas negative tweets tended to receive more replies.
- Neutral tweets made up a lower proportion of tweets that received likes and replies.
...
- We created an LDA model that weighted tweets into 10 topics. For a more complete look at the important words within each topic, view this [screenshot](charts/lda_topics.png) from the third notebook.
    - The table below shows a topic summary and predominant sentiment of each topic and can be used as a reference to the graphs that follow.
...
- Looking at topics across time:

![LDA Topics Across Time](charts/topics_across_time_barstack.png)
<br/><br/>
- There is an interesting spike in topic 7 toward the end of April.
- There is also a large share of topic 0 in the beginning of January, which in our opinion doesn't make much sense and points to some of the limitations of LDA topic modeling.
    - It's possible that this could be relating to the Hong Kong protests during that time, however.
...
- Topic 9 has a lot of negative tweets, which seems to make sense, discussing things like *Trump*, *death*, and social distancing (viewed negatively by some).
...
<br/><br/>
- There was a range of different subjectivity level across topics, but it seems to follow the trend of subjectivity by sentiment and the distribution of sentiment across topics.
...
- Our Bernoulli Naive Bayes model was speedy (taking around 3 minutes to run) and achieved a 71.5% F1 score.

- Our best model was a Decision Tree with 'balanced' ```class_weight``` and a ```ccp_alpha``` of 0.00005; the latter parameter pruned our tree after it was trained. This helped the fitting tremendously and increased our F1 score to 76.7% (the F1 score for our training data was 77.1%). 
    - This model, however, took about an hour and ten minutes to run, so it may not be ideal depending on the use case.
- The model also had a decently high accuracy across each sentiment, which can be seen in the confusion matrix below.

![Decision Tree Confusion Matrix](charts/dt_vec_tuned_cm.png)
<br/><br/>
- Again, word vectors were our most effective features, and these are the top ten:

![Decision Tree Feature Importances](charts/dt_vec_tuned_features.png)
<br/><br/>
- These words are an interesting mix of words that were frequent in positive tweets (*protect*, *help*, *hand*, and *please* were in the top 5 most frequent words for this class) and negative tweets (*infected*, *death*, and *stop* were in the top 5 for this class).
- Interestingly, *fight* was not a very commonly used word and didn't appear in any top 25 list.
...
And finally, the power of words! Words themselves proved to be the most effective predictors of sentiment. While our other features were important during EDA, they did not prove to be effective in our models. In fact, they tended to create noise and lower the efficacy substantially.
